$(function() {
            ...
            $('body').one('pinegrow-ready', function(e, pinegrow) {
                    ...
                    var f = new PgFramework('SemanticUI2.2.4', 'Semantic-UI 2.2.4');
                    ...
                    f.ignore_css_files = [/(^|\/)semantic.*\.(css|less)/i, /(^|\/)text.*\.(css|less)/i, /(^|\/)reset.*\.(css|less)/i];

                    //Auto detect semantic-ui. It can also be manually added / removed from a page with Framework Manager
                    f.detect = function(pgPage) {
                        return pgPage.hasStylesheet(/(^|\/)semantic.*\.(css|less)/i);
                    }
                    f.setScriptFileByScriptTagId('plugin-semanticui-2-2-4'); //get url if script is included directly into edit.html
